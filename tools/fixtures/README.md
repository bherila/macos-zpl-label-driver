...
# qpdf is also required only to regenerate the encrypted-input fixture.
...
19 PDFs / 29 pages plus three self-contained HTML print fixtures. They cover
...
`encrypted-input.pdf` is the native synthetic label wrapped by qpdf in
deterministic password protection. Its public test-only password is recorded in
the fixture manifest so optional source validation can inspect the underlying
synthetic page. Deterministic RC4 is used only to make the committed failure
fixture reproducible; it is not runtime code or a cryptographic recommendation.
