...
def write_annotation_form_pdf():
    """Create an interactive synthetic widget with a real appearance stream."""
    buffer=io.BytesIO();c=canvas.Canvas(buffer,pagesize=(W,H),invariant=1,pageCompression=1)
    c.setTitle('annotation-form')
    payload=draw_label(c,'FORM')
    c.setFillColorRGB(1,1,1);c.rect(17,226,254,46,fill=1,stroke=0)
    c.setFillColorRGB(0,0,0);c.setFont('Helvetica-Bold',8)
    c.drawString(19,258,'INTERACTIVE FORM VALUE - RENDER OR REJECT EXPLICITLY')
    c.acroForm.textfield(
        name='synthetic_reference',
        value='FORM-TEST-123',
        x=19,y=230,width=250,height=22,
        borderColor=black,fillColor=white,textColor=black,
        borderWidth=1,forceBorder=True,fontName='Helvetica',fontSize=10,
    )
    markers(c,W,H);c.showPage();c.save()

    reader=PdfReader(io.BytesIO(buffer.getvalue()))
    fields=reader.get_fields() or {}
    if fields.get('synthetic_reference',{}).get('/V') != 'FORM-TEST-123':
        raise RuntimeError('Synthetic form field tree/value was not created')
    widgets=[]
    for reference in reader.pages[0].get('/Annots',[]):
        annotation=reference.get_object()
        if annotation.get('/Subtype') == '/Widget': widgets.append(annotation)
    if len(widgets) != 1 or not widgets[0].get('/AP') or not widgets[0]['/AP'].get('/N'):
        raise RuntimeError('Synthetic form widget lacks a normal appearance stream')

    writer=PdfWriter();writer.clone_document_from_reader(reader)
    writer.add_metadata({'/Title':'annotation-form','/Author':'Label Printer Driver synthetic fixtures',
                         '/CreationDate':'D:20260915000000Z'})
    path=OUT/'annotation-form.pdf'
    with path.open('wb') as f: writer.write(f)

    reopened=PdfReader(path)
    fields=reopened.get_fields() or {}
    if fields.get('synthetic_reference',{}).get('/V') != 'FORM-TEST-123':
        raise RuntimeError('Written form field tree/value did not survive')
    page=reopened.pages[0];widgets=[]
    for reference in page.get('/Annots',[]):
        annotation=reference.get_object()
        if annotation.get('/Subtype') == '/Widget': widgets.append(annotation)
    if len(widgets) != 1 or not widgets[0].get('/AP') or not widgets[0]['/AP'].get('/N'):
        raise RuntimeError('Written form widget lacks a normal appearance stream')
    box=[float(v) for v in page.cropbox]
    metadata={'mediaBox':[float(v) for v in page.mediabox],'cropBox':box,
              'rotation':0,'userUnit':1,
              'regions':[{'tag':'FORM','payload':payload,'rawRect':[0,0,W,H],
                          'uprightNormalizedRect':[0,0,1,1]}],
              'expectedBarcodePayloads':[payload],'nonLabelPage':False}
    return {'id':'annotation-form','family':'annotation-form','path':str(path.relative_to(REPO)),
            'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'bytes':path.stat().st_size,
            'pageCount':1,'pages':[metadata],'matchExpectation':'explicit-profile',
            'annotationPolicyExpectation':'reject','expectedFormFields':{'synthetic_reference':'FORM-TEST-123'},
            'note':'Interactive widget and AcroForm tree are intentionally unsupported by the renderer.',
            'provenance':'Original synthetic project artwork; generated by tools/fixtures/generate.py',
            'license':'MIT','physicalQualification':'not-run'}
...
def main():
    ...
    fixtures.append(write_annotation_form_pdf())
