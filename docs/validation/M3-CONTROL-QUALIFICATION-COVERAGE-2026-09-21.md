...
| M83 | unknown option keys passed through instead of omitted | AC02 | `ProbeOptionsTests.testAllowedOnly` (+1) |
...
### Part 2 — 12 guards survived, and why the obvious test did not see them

These twelve mutations produced **zero** test failures on the 346-test baseline.
Eleven of them were not guards whose removal let an invalid request succeed
outright: they were guards whose removal left a *later* check to throw a
*different* error, and the existing assertions only demanded "some error". This
is the same shape of trap the 2026-09-20 handoff recorded for M2-AC05, where the
file named after the unit did not cover the guard.

| # | Surviving guard | Clause | Why it survived |
|---|---|---|---|
| M05 | `PrinterProfile`: backfeed `unavailableFeedSpeed`-style capability-state guard | AC01 | With it gone the empty choice set throws `unsupportedBackfeedSpeed` instead. `MotorSpeedIntegrationTests` exercises this distinction for **feed** only. |
| M11 | `PrinterProfile`: tracking `fact.evidence != .unobserved` | AC01 | The only profiles that request tracking in the suite are schema 1 (refused by the schema clause) or hold fully documented facts; the evidence clause never decided an outcome. |
| M12 | `PrinterProfile`: tracking `fact.state == .supported` | AC01 | Same. With unknown facts always carrying `.unobserved` evidence, the adjacent clause caught every case. |
| M13 | `PrinterProfile`: continuous tracking requires an explicit length | AC02 | Redundant with the same rule inside `PhysicalGeometryQualification`, which **is** tested; the profile-layer copy was never the deciding check. |
| M14 | `PrinterProfile`: black-mark tracking requires an explicit offset | AC02 | Same shape: `OffsetControlQualification` holds a tested copy. |
| M16 | `PrinterProfile`: offsets require schema ≥ 6 | AC02 | `OffsetControlIntegrationTests` asserts only that the schema-1 reference throws, not which error; removing the gate still throws, from the unverified range. |
| M33 | `FinishingControlQualification`: `1...64Ki` output-limit declaration | AC02 | `maximumOutputBytes: 0` still throws — as `outputLimit`, not `invalidOutputLimit` — and no test tried a limit above the bound. |
| M41 | `FinishingOutputQualification`: unknown RFID refused | AC01 | The only unknown RFID fact tested carried `.unobserved` evidence, so `documented()` threw first. An unknown fact that cites a real document walked past the removed guard. |
| M42 | `FinishingOutputQualification`: unknown prepeel refused | AC01 | Same. With the guard gone a documented-unknown prepeel resolves to `peelPrepeelNotApplicable`, a positive claim about a mechanism nobody observed. |
| M46 | `FinishingOutputQualification`: `supported()` state check | AC01 | Every "bad" wire fact in the suite was unknown-and-unobserved or `reportedInstallation`, both of which `documented()` rejects. A **documented `unsupported`** fact was never tried. |
| M85 | `PrinterProfile`: schema-5 gate on a physical-geometry declaration | AC02 | No test stores a well-formed geometry declaration in a pre-schema-5 profile. |
| M86 | `PrinterProfile`: schema-6 gate on an offset declaration | AC02 | Same for offsets in a pre-schema-6 profile. |
...
- Dropping is covered from the other side: M59/M70 (an empty request is refused
  rather than becoming a no-op), M83 (`ProbeOptions` keeps unknown keys out of
  its result instead of passing them through), M16/M84/M85/M86/M87/M88/M89/M90
  (a control or declaration that the profile schema does not admit fails as a
  version error rather than being quietly ignored).
