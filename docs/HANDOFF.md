...
## STOP: integrated stock-opening regression

[Failed integrated receipt](validation/M4-STOCK-INTEGRATED-REGRESSION-2026-09-17.md) supersedes any assumption of current full green. Finite full gate at00d1026 exited1:106Python/305Core accelerator both passed;379native debug tests failed4assertions in2configured-stock document-opening tests. Native release/later stages not run. Bootstrap relaxed offline stock admission also served the default configured opening caller. Separate explicit configured-setup and offline-candidate policies; preserve both stock identity and dimensions and retain existing default tests. No more source fix in this round under the own-regression stop rule. Earlier35focused/app success is narrow only. Hosted CI36missing PR heads dispatched; current5success/5running/26queued, exact mapping in /tmp/zpl-missing-ci-poll.json. Principal58PR chain has nonlinear layers31/32/33/35/51/53/57/59; cascading rebase and reconciliation of unpublished finite-extents correction required before merge readiness. No stack registration or merge.

## Explicit stock policy: full software baseline restored

[Full integrated receipt](validation/M4-STOCK-POLICY-INTEGRATED-BASELINE-2026-09-17.md) evaluates clean unpublished77de47c. Default configured stock identity/dimension admission is retained; explicit offline correction is separate and used by saved editor UI. Existing failed tests remain intact; new coverage enumerates both constraints and own-store verification across both caller paths. Finite full native gate terminal exit0:106Python/305Core/380Mac both modes plus oracles/inert/signature/packaged checks. App setup-app.mqWKli is local only. Prior379native failed receipt remains historical; current stock admission blocker removed after full passing verification. New541file exact-source Linux archive/handoff ready, no worker/result. Next whole media/privacy reconciliation and reviewable unpublished source integration; all manual/physical gates and frozen PartB candidate unchanged. Hosted36branch-head CI batch remains separate and does not attach to PR3 rollup. No source push, stack registration/rebase, merge or publishing.
