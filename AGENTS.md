...
A re-seal is a rebind, not a re-verification. Re-hash every cited path and record the count: if all match, `sourceSHA` is the only field that moves. If a cited file changed, that is a new claim and needs its evidence re-run, not a quiet re-hash. Verify currency **after committing**: a dirty worktree reports `WORKSPACE-DIRTY` and can never be current. Confirm on merged `main` that the record survived its own squash merge.
