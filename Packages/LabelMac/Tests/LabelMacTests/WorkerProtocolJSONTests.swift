...
final class WorkerProtocolJSONTests: XCTestCase {
    ...
    func testEveryProtocolIntegerSiteRejectsFractionBeforeCodable() throws {
        ...
        let sites: [(WorkerProtocolJSON.Message, String)] =
            ["schemaVersion", "widthDots", "heightDots", "zplBytes", "previewBytes", "workerMaximumResidentBytes"].map {
                (.renderResult, "{\"" + $0 + "\":" + fraction + "}")
            } + [(.failure, "{\"schemaVersion\":\(fraction)}"),
                 (.layoutRequest, "{\"schemaVersion\":\(fraction)}"),
                 (.layoutRequest, "{\"maximumPages\":\(fraction)}"),
                 (.layoutRequest, "{\"structuralPages\":[1,\(fraction)]}"),
                 (.layoutRequest, "{\"barcodePages\":[1,\(fraction)]}"),
                 (.layoutResult, "{\"schemaVersion\":\(fraction)}"),
                 (.layoutResult, "{\"pages\":[{\"rotation\":0},{\"rotation\":\(fraction)}]}")]
